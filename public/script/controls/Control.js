define(['lib/async', 'control/Observable', 'control/Resizable', 'control/Draggable', 'control/Editable'/*, 'css!/css/control'*/],
    function (async, observable, resizable, draggable, editable) {
        ...
        var Control = function (config, callback) {
            ...
            this.addItems(config.items || [], function (err) {
                ...
                if (config.mixins) {
                    for (var i = 0, l = config.mixins.length; i < l; i++) {
                        config.mixins[i].make.self;
                    }
                }
                callback(err, self);
        ...
        Control.prototype.getPreviousSibling = function () {
            var result = null,
                myIndexInParent = this.parentControl.items.indexOf(this);
            if (myIndexInParent > 0) {
                result = this.parentControl.items[myIndexInParent - 1];
            }
            return result;
        };

        Control.prototype.getNextSibling = function () {
            var result = null,
                myIndexInParent = this.parentControl.items.indexOf(this);
            if (myIndexInParent > -1 && myIndexInParent < this.parentControl.items.length - 1) {
                result = this.parentControl.items[myIndexInParent + 1];
            }
            return result;
        };
        ...
        Control.create = function (type, config, callback) {
            if (typeof type === 'function') {
                ...
                require(['control/' + ( type.replace('.', '/'))], function (requiredType) {
